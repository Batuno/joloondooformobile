    ...
    if request.method == 'POST':
        try:
            ...
            if user_data and hashed_password == user_data[3]:
                new_token = hashlib.sha256(os.urandom(60)).hexdigest().encode('ascii').decode()
                cur.execute(
                    """
                    INSERT INTO tbl_usertokens (user_id, token)
                    VALUES (%s, %s)
                    """,
                    (user_data[0], new_token)
                )
                con.commit()
                print("login suc")
...
def getUser(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # user_id = request.session.get('user_id') ###session shaardana odoohondoo arilgachiy
...
def submit_exam(request):
    ...
    if request.method == 'POST':
        try:
            ...
            cur = con.cursor()
            cur.execute("""
            SELECT user_id FROM tbl_usertokens WHERE token = %s
            """, [token])
            user_id = cur.fetchone()[0]
            if user_id is None:
                response_data = {
...
def track_user(request, user_id):
    ...
    if request.method == 'GET':
        try:
            ...
            response_data = { 
                "message": "Successfully fetched user exam history.",
...
#             cur.execute(
#                 """
#                 SELECT q.question_id, q.q_images, q_explanation, a.answer_id, a.a_text, a.a_iscorrect, ua.answer_id AS useranswer_id
#                 FROM tbl_question q
#                 INNER JOIN tbl_answer a ON q.question_id = a.question_id
#                 LEFT JOIN tbl_useranswer ua ON q.question_id = ua.question_id AND ua.exam_id = %s AND ua.user_id = %s
#                 WHERE ua.exam_id = %s
...
#             score = 0
#             for row in rows:
#                 question_id, q_images, q_explanation, answer_id, a_text, a_iscorrect, useranswer_id = row
#                 if question_id not in questions:
#                     questions[question_id] = {
#                         'question_id': question_id,
#                         'image': q_images,
...
#                     }
#                 user_answer = useranswer_id == answer_id
#                 if user_answer and a_iscorrect:
#                     score += 1
#                 questions[question_id]['answers'].append({
#                     'answer_id': answer_id,
#                     'text': a_text,
